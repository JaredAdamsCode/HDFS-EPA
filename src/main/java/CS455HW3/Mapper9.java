...
public class Mapper9 extends Mapper<LongWritable, Text, Text, DoubleWritable>{
...
//	}
	...
	protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException{
		...
		if(delimittedLine.length > 0) {
			...
			String gmt = delimittedLine[12].replace("\"","");
			gmt = gmt.substring(0, 2);
			boolean valid = numberCheck(rawMeasurement);
			Double measurement = 0.0;
			if(valid) {
				measurement = Double.parseDouble(delimittedLine[13].replace("\"",""));
			}
			String year = delimittedLine[11].substring(0, 5).replace("\"", "");
			boolean validYear = numberCheck(rawMeasurement);
			Double yearDouble = 0.0;
			if(validYear) {
				yearDouble = Double.parseDouble(year);
			}

			if(valid && validYear && yearDouble >= 2000) {
				context.write(new Text(gmt), new DoubleWritable(measurement));
